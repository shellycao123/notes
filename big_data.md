...
2. run on the same node as Hadoop process. no mapreduce.
...
2. column oriented, semi-structured data
...
3. support batch processing, streaming,ML, graphx for parallel execution of graph algorithms(RDD).
...
10. worker nodes a coordinated by cluster manage, which then communicates with the driver program(spart context)
11. fault tolerance: auto dels with failed or slow machines by re-executing the tasks. 
12. runs on top of JVM, can use tools built for JVM stack. 
13. REPL: read/evaluate/print loop. every spark app needs a sparkcontext. provides a preconfigured context sc. 
14. RDD: resilient distributed dataset, 
    ...
    - types of operations: 
      ...
      - RDD can hold any type of data
15. Spark can choose to perform transformations by row. don't need to process on every row, like mapreduce.
16. use parallelize to create RDD from memory. filebased use sc.textFile, maps each line to its own RDD element, only works with newline separated text files. sc.wholeTextFiles map entire content of all files in a directory to a single RDD element, only work for small files fit into memory, can be used fro json, xml etc. 
17. Spark SQL: structured data processing. 
...
3. RDD: core spark objects, don't have a senses of columns or structure. 
4. dataframe: is strcutured, more auto optimization, but don't support complie time type safty. 
5. dataset: similar to dataframes, can be wrapped in explicit type and structure, schema can be upfront at compile time. type related errors will be detected when you build your script instead of while running. type safe and object oriented. 
6. data frame queries must return new dataframe. 
7. Spark SQL is built on Spard, a general purposed processing engine, impala is a specialized SQL engine. 
...
4. using streaming context. new API based on dataset and dataframes, not micro-batch set RDD. Dstream(Dsicretized Stream), infact continuous RDDs
5. transformations & output operations. 


## zookeeper
1. distributedly coordinate between nodes. keep track of info that need to be in sync across the cluster. can be used to recover from partial failure. share info across different hadoop services. 
...
6. YARN: resource management in hadoop 2, separate processing from resource management.
   1. can run workflows other than MapReduce, ex: Spark
   2. resource manager: 
